  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
      ...
      <div className="clay-card p-5 flex items-center gap-4" style={{ boxShadow: "inset 0 4px 6px rgba(255,255,255,0.9), 0 20px 40px -10px rgba(72,187,120,0.2)" }}>
        ...
        <div>
          <p className="font-heading font-extrabold text-3xl text-green-500">87</p>
          <p className="text-xs font-body text-muted-foreground">Today's Score</p>
          <p className="text-[10px] font-body text-green-500">Great day so far!</p>
        </div>
      </div>
        ...
        <div className="relative w-14 h-14">
          <svg className="w-full h-full -rotate-90" viewBox="0 0 100 100">
            ...
            <circle cx="50" cy="50" r="42" fill="none" stroke="hsl(var(--primary))" strokeWidth="8" strokeLinecap="round"
              strokeDasharray={`${(4/6) * 2 * Math.PI * 42} ${2 * Math.PI * 42}`} />
          </svg>
          <span className="absolute inset-0 flex items-center justify-center text-xs font-heading font-bold">4/6</span>
        </div>
        <div>
          <p className="font-heading font-extrabold text-xl text-foreground">4/6 completed</p>
      ...
      <div className="clay-card p-5 flex items-center gap-4" style={{ boxShadow: "inset 0 4px 6px rgba(255,255,255,0.9), 0 20px 40px -10px rgba(72,187,120,0.15)" }}>
        ...
        <div>
          <p className="font-heading font-extrabold text-3xl text-emerald-500">3.5</p>
